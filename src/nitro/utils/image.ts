...
export const validateImageFile = async (file: Blob, acceptGif?: boolean, ignoreFileSize?: boolean) => {
	if (!(await isAcceptedImageFile(file, acceptGif))) return createApiErrorAndThrow(400);
	if (!ignoreFileSize && file.size > 1048576 * 16) createApiErrorAndThrow(413); // Max 16MB
};
